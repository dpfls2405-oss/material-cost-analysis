...
col3.metric("BOM 예상소요금액", f"{month_df['expected_usage_amount'].fillna(0).sum():,.0f}")
gap_amount_total = month_df["usage_gap_amount"].fillna(0).sum()
col4.metric(
    "구매-예상 금액차이 합계",
    f"{gap_amount_total:,.0f}",
    delta=f"{gap_amount_total:,.0f}",
    delta_color="inverse",
)
...
with tab1:
    ...
    valid_qty = month_df["usage_gap_qty"].notna().sum()
    st.info(f"수량 GAP 계산 가능 자재: {valid_qty:,}개")

    if valid_qty > 0:
        st.plotly_chart(
            bar_material_gap(month_df, "수량 GAP TOP 20 (|실사용량 - BOM예상| 기준)"),
            use_container_width=True,
        )

    qty_cols = [
        "material_id", "material_name",
        "begin_qty", "purchase_qty", "calculated_end_qty",
        "actual_usage_qty", "expected_usage_qty", "usage_gap_qty",
    ]
    qty_existing = [c for c in qty_cols if c in month_df.columns]
    st.dataframe(
        month_df[qty_existing]
        .dropna(subset=["usage_gap_qty"])
...
with tab2:
    ...
    valid_amt = month_df["usage_gap_amount"].notna().sum()
    st.info(f"금액 GAP 계산 가능 자재: {valid_amt:,}개")

    if valid_amt > 0:
        st.plotly_chart(
            bar_material_gap_amount(month_df, "금액 GAP TOP 20 (|구매금액 - BOM예상금액| 기준)"),
            use_container_width=True,
        )
    ...
    amt_existing = [c for c in amt_cols if c in month_df.columns]
    st.dataframe(
        month_df[amt_existing]
...
with tab3:
    ...
    if bom_df.empty:
        st.info("BOM 예상소요량 데이터가 없습니다. BOM과 입고실적을 업로드해 주세요.")
    else:
        st.plotly_chart(
            bar_bom_expected(bom_df, "BOM 예상소요량 TOP 20"),
            use_container_width=True,
        )
        ...
        bom_existing = [c for c in bom_cols if c in bom_df.columns]
        st.dataframe(
            bom_df[bom_existing]
            .sort_values("expected_usage_amount", ascending=False),
            use_container_width=True,
        )
